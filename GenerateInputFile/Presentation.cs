            ...
            try
            {
                using (ClientContext clientContext = new ClientContext(ConfigurationManager.AppSettings.Get(SPOConstants.SPOSiteURL)))
                {
                    ...
                    foreach (var obj in items)
                    {
                        ...
                        dtResponse.Rows.Add(Constants.Ir_presentation, documentum_r_object_id, documentum_i_chronicle_id, Constants.Ir_presentation_r_folder_path, Name, FileFormatConstants.PDF, Constants.Presentations + "/" + Name, guid, guid, guid + '@' + guid, Constants.Presentations, display_order,"NULL");
        ...
        private void GetChildItem(ClientContext clientContext, string Id, string ParentGuid, string i_chronicle_id, string r_object_id, string content_id, string display_order, string ParentFileName, ref DataTable dtResponse)
        {
            ...
            try
            {
                ...
                foreach (var obj in items)
                {
                    ...
                    dtResponse.Rows.Add(Constants.Ir_presentation, r_object_id, i_chronicle_id, Constants.Ir_presentation_r_folder_path, ParentFileName, FileFormatConstants.JPEG, Constants.Presentations + "/" + Name, ParentGuid, ParentGuid, guid + '@' + ParentGuid, Constants.Presentations, display_order,"NULL");
                }
            }
