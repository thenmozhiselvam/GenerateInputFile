            ...
            try
            {
                using (ClientContext clientContext = new ClientContext(ConfigurationManager.AppSettings.Get(SPOConstants.SPOSiteURL)))
                {
                    ...
                    foreach (var obj in items)
                    {
                        ...
                        dtResponse.Rows.Add(Constants.Ir_8k_filing, documentum_r_object_id, documentum_i_chronicle_id, Constants.EightK_Other_Filing_r_folder_path, Name, FileFormatConstants.PDF, Constants.EightK_Other_Filing + "/" + Name, guid, guid, guid + '@' + guid, Constants.EightK_Other_Filing, display_order,"NULL");
